...
export default function DiscoverVenueCard({ venue }: DiscoverVenueCardProps) {
  return (
    <article className="group overflow-hidden rounded-[1.6rem] border border-white/10 bg-white/5 shadow-[0_24px_70px_rgba(0,0,0,0.3)] backdrop-blur-xl transition duration-300 hover:-translate-y-1 hover:border-cyan-400/30 hover:shadow-[0_24px_80px_rgba(0,0,0,0.36)]">
      <div className={`relative h-52 bg-gradient-to-br ${venue.imageClass}`}>
      ...
      <div className="space-y-4 p-5">
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-zinc-400">{venue.tagline}</p>
          </div>
          <span className="rounded-full border border-white/15 bg-white/10 px-2.5 py-1 text-xs font-medium text-zinc-200">
            {venue.crowdLevel}
          </span>
        </div>
        ...
        <div className="grid gap-2 rounded-[1.25rem] border border-white/10 bg-black/20 p-3 text-sm text-zinc-300">
          ...
          <div className="flex items-center justify-between">
            <span>Hours</span>
            <span className="font-medium text-white">{venue.hours}</span>
          </div>
        </div>

        <div className="flex flex-col gap-2 sm:flex-row">
          <Link href="#" className="flex-1 rounded-full border border-white/15 bg-white/5 px-4 py-2 text-center text-sm font-medium text-zinc-100 transition hover:border-cyan-400/40 hover:bg-white/10">
            Details
          </Link>
          <Link href="#" className="flex-1 rounded-full bg-gradient-to-r from-cyan-500 to-violet-500 px-4 py-2 text-center text-sm font-medium text-white transition hover:opacity-90">
            Directions
          </Link>
        </div>
      </div>
