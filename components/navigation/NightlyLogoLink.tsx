...
export default function NightlyLogoLink({
  ...
  const href = roleHomeHref[activeRole];

  return (
    <Link
      href={href}
      aria-label="Go to your Nightly home"
      className={`relative z-10 inline-flex cursor-pointer items-center rounded-2xl px-2 py-2 transition duration-200 ease-out hover:-translate-y-0.5 hover:bg-white/5 hover:drop-shadow-[0_0_14px_rgba(74,222,255,0.28)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300/50 ${className ?? ""}`}
    >
      <Image
